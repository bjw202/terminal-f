...
| Claude Code 등이 스트리밍 출력 중 한글 조합이 깨짐(자모는 보이나 완성이 이상) | xterm은 매 렌더마다 `CompositionHelper.updateCompositionElements()`로 숨은 IME textarea를 커서로 재배치 → 초당 수십 번 출력→렌더가 조합 중 textarea를 흔들어 `value.substring` 기반 확정이 어긋남. **해결: 조합 중(`compositionstart`~`compositionend`)에는 해당 pane의 PTY 출력을 per-view 버퍼에 보류하고 compositionend에서 한 번에 flush**(`terms.writeOutput`이 유일한 출력 sink). 안전장치: 워치독(1s, 조합 방치 시 강제 flush 후 이번 조합 버퍼링 포기) + 크기 캡(256KB 초과 즉시 flush) + **에코 통과창(ECHO_PASS_MS=120ms: onData로 PTY에 쓴 직후 도착한 출력은 방금 확정한 음절의 에코/재그리기라 조합 중에도 즉시 렌더 — 전부 보류하면 연속 타이핑 시 다음 음절 조합이 직전 에코를 가둬 커서 정체+IME 미리보기가 한 칸에 겹침, 실기기 회귀로 발견. 통과 write는 항상 버퍼 먼저 flush해 순서 보존)** + blur 방어(조합 중 blur 시 compositionend 미발생으로 플래그 고착→Ctrl+C/V 죽는 2차 버그, blur에서 플래그 리셋+flush). 순서 불변식: 버퍼 비어있지 않은 동안 직접 write 금지. IME 실확정은 합성이벤트로 검증 불가→실기기 필수 |
