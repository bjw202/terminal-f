...
- 레이아웃: **좌측 사이드바 기본 (확정, 2026-07-02)** — 단 **접기/펴기
  ...
  와 설정 버튼 고정.
- **워크스페이스 시작 폴더(root folder)** — **구현 완료 (2026-08-06,
  SPEC-WORKSPACE-ROOT-001)**. 우클릭 컬러 메뉴를 확장해 네이티브 폴더
  선택 대화상자(`rfd`)로 워크스페이스별 시작 폴더를 지정할 수 있다.
  설정하면 해당 워크스페이스의 모든 pane cwd가 그 폴더로 일괄 재작성되고,
  재시작 후부터 그 폴더에서 셸이 열린다. 자세한 설계 결정은
  [ADR-013](ADR-013-workspace-root-folder.md) 참고.
